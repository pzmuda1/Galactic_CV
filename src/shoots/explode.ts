import { boardElement } from "../board/boardManager";
import "./explode.scss";
...
export function sparcle({ top, left }: { top: number; left: number }) {
  const explosions: HTMLDivElement[] = [];

  explosions.push(exolpodeGroup(left, top, { scale: 1, x: -50, y: -50, r: 0 }));
  explosions.push(
    exolpodeGroup(left, top, {
      scale: 0.5,
      x: -30,
      y: -50,
      r: 180,
